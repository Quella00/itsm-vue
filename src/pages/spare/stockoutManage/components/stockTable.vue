<template>
    <div>
        ...
        <div class="tabs-panel-wrap border mb15">
            <div class="panel-header" style="line-height:43px;">
                ...
                <span class="fr">
                        ...
                        <el-button type="primary" icon="el-icon-my-add" size="normal" @click="spareAddHandler" v-if="type=='edit'" >添加出库设备</el-button>
                        <el-button type="primary" icon="el-icon-my-reset" size="normal" @click="resetHandler">重置</el-button>
                    </span>
            </div>
            <div class="panel-body">
                <el-table :data="tableConfig.data" max-height="270" cell-class-name="resetCell" @selection-change="selectionChangeHandler"
                    header-cell-class-name="resetHeaderCell">
        ...
        <!-- 添加返库备件弹窗 -->
        <spareDialog v-model="spareDialog.show" v-if="spareDialog.show" :query="tableConfig.message" @callback="saveSpareDialog"
            :defaultSelect="tableConfig.cacheData"></spareDialog>
    </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getTableFun(params) {
                params = params || {
                    warehouseOutId: this.$route.query.id,
                ...
                this.$axios.post('/itsm/device/out/correlate/query', params).then((res) => {
                    if (res.status == 200) {
        ...
        watch: {
            "tableConfig.cacheData": {
                handler(newVal, oldVal) {
                    ...
                    this.tableConfig.pagination.total = newVal.length;
                },
                deep: true
            }
        }
