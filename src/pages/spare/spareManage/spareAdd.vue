                            ...
                            <el-collapse-item name="1">
                                ...
                                <el-form label-width="135px" :model="basicInfoForm" :rules="basicInfoRules" ref="basicInfoForm" :inline="true" style="margin-top: 20px;">
                                    ...
                                    <el-form-item label="入库单：" prop="warehouseInNumber">
                                        <el-input v-model="basicInfoForm.warehouseInNumber" placeholder="请输入入库单"></el-input>
                                    </el-form-item>
        ...
        methods: {
            ...
            **/
            submit(type) {
                ...
                if (flag) {
                    var param = $.extend({}, {submitAudit: type=='audit'?true:false},this.basicInfoForm, this.maintenanceInfoForm, this.extendedAttributeForm, this.financeInfoForm);
                    this.submitForm(param);
                } else {
                    this.$message.warning('表单输入有误，请检查');
                }
            },
            /*
            * 提交表单
            **/
            submitForm(param) {
                this.$ies.formatObject(param,[]);
                this.$axios.post('/itsm/device/add', param).then((res) => {
                    if (res.status == 200) {
                        this.$message.success('新增备件成功!');
                        this.$ies.dispatch(this, 'layout').$refs.tabsnav.closePage('/spare/spareAdd');
                        ...
                        this.$message({ type: 'error', message: '新增备件失败' });
                    }
                });
            }
        }
