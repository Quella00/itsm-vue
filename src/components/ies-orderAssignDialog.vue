<template>
    <!-- 关闭工单弹窗 -->
    <el-dialog :visible="true" :close-on-click-modal='false' custom-class="orderAssignDialog" @close="closeHandler" v-drag>
        <span slot="title" style="color:#666666;">指派工单</span>

        <el-form label-width="165px" :rules="rules" ref="orderAssignId" :model="params">
            <el-form-item label="下一节点：" prop="processNodeName">
                {{params.processNodeName}}
            </el-form-item>
            <el-form-item label="角色类型：" prop="processRoleName">
                {{params.processRoleName}}
            </el-form-item>
            <el-form-item label="指派至：" prop="nextUserId">
                <el-select v-model="params.nextUserId" placeholder="请选择">
        ...
        data() {
            return {
                ...
                rules: {
                    ...
                    content:[
                        { required: true, message: '请输入指派说明', trigger: 'change' },
        ...
        methods: {
            init() {
                ...
                if (this.data) {
                    ...
                    this.$axios.post('/itsm/system/permission/role/queryOrdinaryUserByRoleId', { roleId: this.params.processRoleId }).then((res) => {
                        if (res.status == 200) {
                            this.users = res.data;
                        }
                    });
            ...
            closeHandler(type) {
                if (type == 'confirm') {
                    ...
                    this.$refs['orderAssignId'].validate((valid) => {
                        if(valid){
                            ...
                            this.$axios.post('/itsm/process/manager/node/action/appoint', params).then((res) => {
                                if (res.status == 200) {
                                    this.$emit('input', '');
